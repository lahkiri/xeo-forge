...
The Windows v1.4.0 installer uses a per-user one-click NSIS configuration and calls the updater in silent mode. **Restart to update** should close the old process, install the downloaded version without reopening an interactive setup wizard, and launch the new version directly. The continuity gate still requires a real installed-device test from v1.3.1: discovery, user-approved download, restart, version change, and preservation of the SQLite database, Local Owner, settings, task history, and project path.
