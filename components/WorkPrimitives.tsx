      ...
      >
        <span
          className={cx(
            ...
            ok === undefined ? 'animate-pulse bg-cyan-300' : ok ? 'bg-emerald-400/80' : 'bg-red-400/80',
          )}
          aria-label={ok === undefined ? 'running' : ok ? 'succeeded' : 'failed'}
        />
        <Badge tone={meta.tone} className="shrink-0 font-mono">{meta.label}</Badge>
        {args && <span className="truncate font-mono text-[11px] text-gray-500">{args}</span>}
        <span className="ml-auto shrink-0 text-[10px] tabular-nums text-gray-700">{formatClock(call.ts)}</span>
        {expandable && (
          <span aria-hidden="true" className={cx('shrink-0 text-[9px] text-gray-600 transition', open && 'rotate-90')}>
            ▶
          </span>
        )}
      </button>
      {open && (
        <pre className="max-h-56 overflow-auto border-t border-white/[0.04] bg-black/25 px-3 py-2 font-mono text-[11px] leading-5 text-gray-400">
          {body.slice(0, 4000)}
          {body.length > 4000 && '\n…truncated'}
        </pre>
      )}
...
export function PlanReview({
  ...
  return (
    <div className="flex min-h-0 flex-1 flex-col">
      <div className="shrink-0 border-b border-amber-300/15 bg-amber-300/[0.06] px-4 py-3">
        <div className="flex flex-wrap items-center justify-between gap-3">
          <div className="min-w-0">
            ...
            <p className="mt-0.5 text-[12px] leading-5 text-gray-400">
        ...
        {rejecting && (
          <div className="mt-3 rounded-lg border border-white/10 bg-black/25 p-3">
            <label htmlFor="reject-reason" className="mb-1.5 block text-[11px] font-medium text-gray-400">
              What should change?
            </label>
            <textarea
              id="reject-reason"
              ...
              className="w-full resize-none rounded-lg border border-white/10 bg-[#0c1320]/80 px-3 py-2 text-[12px] leading-5 text-gray-100 outline-none focus:border-cyan-300/40"
            />
      ...
      <div className="min-h-0 flex-1 overflow-y-auto px-4 py-4">
        <pre className="whitespace-pre-wrap font-sans text-[13px] leading-6 text-gray-300">{plan}</pre>
      </div>
    </div>
  );
}
...
export function DecisionGate({
  ...
  return (
    <div className="border-b border-violet-300/20 bg-violet-300/[0.07] px-4 py-4">
          ...
          <div className="flex items-center gap-2">
            ...
            <span
              className={cx(
                ...
                urgent ? 'bg-red-400/20 text-red-200' : 'bg-white/[0.08] text-gray-300',
              )}
            >
              {seconds}s
            </span>
          </div>
          <p className="mt-1 max-w-xl text-[12px] leading-5 text-gray-400">
