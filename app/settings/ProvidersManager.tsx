...
export default function ProvidersManager() {
  ...
  // v1.25: edit-provider and edit-model flows. The stored API key NEVER
  // round-trips to the client (api_key_set is the only signal), so the edit
  // form's key field stays blank = "keep the stored key".
  const [showEditProviderForm, setShowEditProviderForm] = useState(false);
  const [editProviderForm, setEditProviderForm] = useState({ name: '', baseUrl: '', apiKey: '' });
  const [editingModelId, setEditingModelId] = useState('');
  const [editModelForm, setEditModelForm] = useState({ name: '', modelId: '', temperature: '0.7', maxTokens: '4000', contextWindow: '128000' });

  function openEditProvider(provider: NonNullable<typeof selectedProvider>) {
    setShowEditProviderForm(true);
    setShowProviderForm(false);
    setEditProviderForm({ name: provider.name, baseUrl: provider.base_url, apiKey: '' });
  }

  async function saveProviderEdit(event: React.FormEvent) {
    event.preventDefault();
    if (!selectedProvider) return;
    const payload: Record<string, unknown> = {};
    if (editProviderForm.name.trim() && editProviderForm.name.trim() !== selectedProvider.name) payload.name = editProviderForm.name.trim();
    if (editProviderForm.baseUrl.trim() && editProviderForm.baseUrl.trim() !== selectedProvider.base_url) payload.baseUrl = editProviderForm.baseUrl.trim();
    if (editProviderForm.apiKey.trim()) payload.apiKey = editProviderForm.apiKey.trim();
    if (Object.keys(payload).length === 0) {
      setNotice({ tone: 'ok', text: 'Nothing changed.' });
      setShowEditProviderForm(false);
      return;
    }
    await mutate(() => requestProvider(selectedProvider.id, payload), 'Provider updated. The stored key is untouched unless a new one was entered.');
    setShowEditProviderForm(false);
    setEditProviderForm({ name: '', baseUrl: '', apiKey: '' });
  }

  function openEditModel(model: ProviderModel) {
    setEditingModelId(model.id);
    setShowModelForm(false);
    setEditModelForm({ name: model.name, modelId: model.model_id, temperature: String(model.temperature), maxTokens: String(model.max_tokens), contextWindow: String(model.context_window) });
  }

  async function saveModelEdit(event: React.FormEvent) {
    event.preventDefault();
    if (!selectedProvider || !editingModelId) return;
    await mutate(
      () => requestModel(selectedProvider.id, editingModelId, {
        name: editModelForm.name.trim(),
        modelId: editModelForm.modelId.trim(),
        temperature: Number(editModelForm.temperature),
        maxTokens: Number(editModelForm.maxTokens),
        contextWindow: Number(editModelForm.contextWindow),
      }),
      'Model updated. Running sessions keep their loaded credentials; the next run uses the new settings.',
    );
    setEditingModelId('');
  }
  ...
  return (
    <div className="settings-page">
      ...
      {showProviderForm && <form className="settings-form-card" onSubmit={addProvider}><div className="settings-form-heading"><div><span className="codex-kicker">New connection</span><h3>Add provider</h3></div><button type="button" className="settings-close inline-flex" onClick={() => setShowProviderForm(false)}><IconX size={12} /></button></div><div className="settings-form-grid"><label>Name<input className="settings-input" value={providerForm.name} onChange={(event) => setProviderForm({ ...providerForm, name: event.target.value })} placeholder="OpenAI" required /></label><label>Slug<input className="settings-input" value={providerForm.slug} onChange={(event) => setProviderForm({ ...providerForm, slug: event.target.value })} placeholder="openai" required /></label><label className="settings-form-wide">Base URL<input className="settings-input" type="url" value={providerForm.baseUrl} onChange={(event) => setProviderForm({ ...providerForm, baseUrl: event.target.value })} required /></label><label className="settings-form-wide">API key<input className="settings-input" type="password" value={providerForm.apiKey} onChange={(event) => setProviderForm({ ...providerForm, apiKey: event.target.value })} placeholder="Stored securely on this runtime" /></label></div><div className="settings-form-actions"><Button type="button" variant="secondary" onClick={() => setShowProviderForm(false)}>Cancel</Button><Button type="submit" loading={busy}>Create provider</Button></div></form>}
      {showEditProviderForm && selectedProvider && <form className="settings-form-card" onSubmit={saveProviderEdit}><div className="settings-form-heading"><div><span className="codex-kicker">Edit connection</span><h3>Edit {selectedProvider.name}</h3><p className="settings-form-help">Live sessions are never broken: a running agent keeps the credentials it loaded at start; the next run picks up changes.</p></div><button type="button" className="settings-close inline-flex" onClick={() => setShowEditProviderForm(false)}><IconX size={12} /></button></div><div className="settings-form-grid"><label>Name<input className="settings-input" value={editProviderForm.name} onChange={(event) => setEditProviderForm({ ...editProviderForm, name: event.target.value })} required /></label><label className="settings-form-wide">Base URL<input className="settings-input" type="url" value={editProviderForm.baseUrl} onChange={(event) => setEditProviderForm({ ...editProviderForm, baseUrl: event.target.value })} required /></label><label className="settings-form-wide">API key<input className="settings-input" type="password" value={editProviderForm.apiKey} onChange={(event) => setEditProviderForm({ ...editProviderForm, apiKey: event.target.value })} placeholder={selectedProvider.api_key_set ? 'Leave blank to keep the stored key' : 'No key stored yet'} autoComplete="new-password" /></label></div><div className="settings-form-actions"><Button type="button" variant="secondary" onClick={() => setShowEditProviderForm(false)}>Cancel</Button><Button type="submit" loading={busy}>Save changes</Button></div></form>}
      {editingModelId && selectedProvider && selectedProvider.models.some((model) => model.id === editingModelId) && <form className="settings-form-card settings-model-form" onSubmit={saveModelEdit}><div className="settings-form-heading"><div><span className="codex-kicker">Edit model</span><h3>Edit {selectedProvider.models.find((model) => model.id === editingModelId)?.name}</h3></div><button type="button" className="settings-close inline-flex" onClick={() => setEditingModelId('')}><IconX size={12} /></button></div><div className="settings-form-grid"><label>Display name<input className="settings-input" value={editModelForm.name} onChange={(event) => setEditModelForm({ ...editModelForm, name: event.target.value })} required /></label><label>Model ID<input className="settings-input" value={editModelForm.modelId} onChange={(event) => setEditModelForm({ ...editModelForm, modelId: event.target.value })} required /></label><label>Temperature<input className="settings-input" type="number" min="0" max="2" step="0.1" value={editModelForm.temperature} onChange={(event) => setEditModelForm({ ...editModelForm, temperature: event.target.value })} /></label><label>Max output tokens<input className="settings-input" type="number" min="256" value={editModelForm.maxTokens} onChange={(event) => setEditModelForm({ ...editModelForm, maxTokens: event.target.value })} /></label><label>Context window<input className="settings-input" type="number" min="1024" value={editModelForm.contextWindow} onChange={(event) => setEditModelForm({ ...editModelForm, contextWindow: event.target.value })} /></label></div><div className="settings-form-actions"><Button type="button" variant="secondary" onClick={() => setEditingModelId('')}>Cancel</Button><Button type="submit" loading={busy}>Save model</Button></div></form>}
