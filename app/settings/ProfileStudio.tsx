  ...
  return (
    <Card className="mb-6">
      <div className="mb-5 flex flex-wrap items-start justify-between gap-3">
        <div>
          <p className="text-meta font-semibold uppercase tracking-[0.18em] text-purple-300">PROFILE STUDIO</p>
      ...
      <form onSubmit={create} className="grid gap-3 md:grid-cols-[1fr_150px]">
        <input value={name} onChange={(e) => setName(e.target.value)} placeholder="Profile name, e.g. Senior Builder" className="rounded-md border border-line bg-ink-700/60 px-3 py-2 text-ui outline-none placeholder:text-content-muted focus:border-purple-400/50" />
        <select value={kind} onChange={(e) => setKind(e.target.value as AgentProfileKind)} className="rounded-md border border-line bg-[#111419] px-3 py-2 text-ui text-content-secondary outline-none focus:border-purple-400/50">{kinds.map((item) => <option key={item} value={item}>{item}</option>)}</select>
        <input value={description} onChange={(e) => setDescription(e.target.value)} placeholder="Short description" className="rounded-md border border-line bg-ink-700/60 px-3 py-2 text-ui outline-none placeholder:text-content-muted focus:border-purple-400/50 md:col-span-2" />
        <textarea value={instructions} onChange={(e) => setInstructions(e.target.value)} rows={3} placeholder="You are a senior engineer. Prefer small reversible changes, run tests, and explain trade-offs…" className="w-full resize-y rounded-md border border-line bg-ink-700/60 px-3 py-2 text-ui leading-6 outline-none placeholder:text-content-muted focus:border-purple-400/50 md:col-span-2" />
