    ...
    <Card className="mb-6">
      <div className="mb-5 flex flex-wrap items-start justify-between gap-3">
        <div>
          ...
          <p className="mt-1 max-w-2xl text-xs leading-5 text-gray-500">Create a focused operating mode for building, research, analysis, or operations. Profiles guide behavior; they never grant permissions or bypass approval.</p>
        </div>
        <span className="rounded-full bg-purple-500/10 px-2.5 py-1 text-[10px] text-purple-300">{profiles.filter((p) => p.enabled).length} enabled</span>
      </div>
      ...
      <form onSubmit={create} className="grid gap-3 md:grid-cols-[1fr_150px]">
        <input value={name} onChange={(e) => setName(e.target.value)} placeholder="Profile name, e.g. Senior Builder" className="rounded-md border border-white/10 bg-white/[0.03] px-3 py-2 text-sm outline-none placeholder:text-gray-600 focus:border-purple-400/50" />
        <select value={kind} onChange={(e) => setKind(e.target.value as AgentProfileKind)} className="rounded-md border border-white/10 bg-[#111419] px-3 py-2 text-sm text-gray-300 outline-none focus:border-purple-400/50">{kinds.map((item) => <option key={item} value={item}>{item}</option>)}</select>
        <input value={description} onChange={(e) => setDescription(e.target.value)} placeholder="Short description" className="rounded-md border border-white/10 bg-white/[0.03] px-3 py-2 text-sm outline-none placeholder:text-gray-600 focus:border-purple-400/50 md:col-span-2" />
        <textarea value={instructions} onChange={(e) => setInstructions(e.target.value)} rows={3} placeholder="You are a senior engineer. Prefer small reversible changes, run tests, and explain trade-offs…" className="w-full resize-y rounded-md border border-white/10 bg-white/[0.03] px-3 py-2 text-sm leading-6 outline-none placeholder:text-gray-600 focus:border-purple-400/50 md:col-span-2" />
        <div className="md:col-span-2"><Button type="submit" disabled={busy || !name.trim() || !instructions.trim()}>Create profile</Button></div>
      </form>
      {profiles.length > 0 && <div className="mt-5 grid gap-3 md:grid-cols-2">{profiles.map((profile) => <div key={profile.id} className="rounded-lg border border-white/[0.07] bg-white/[0.02] p-4"><div className="flex items-start justify-between gap-3"><div><p className="text-sm font-medium text-gray-200">{profile.name}</p><p className="mt-1 text-[10px] uppercase tracking-wider text-gray-600">{profile.kind} · v{profile.version}</p></div><span className={`rounded-full px-2 py-1 text-[10px] ${profile.enabled ? 'bg-green-500/15 text-green-300' : 'bg-white/10 text-gray-500'}`}>{profile.enabled ? 'enabled' : 'disabled'}</span></div><p className="mt-3 line-clamp-3 text-xs leading-5 text-gray-500">{profile.description || profile.instructions}</p><div className="mt-3 flex gap-2"><Button variant="ghost" disabled={busy} onClick={() => void toggle(profile)}>{profile.enabled ? 'Disable' : 'Enable'}</Button><Button variant="ghost" disabled={busy} onClick={() => void remove(profile)}>Delete</Button></div></div>)}</div>}
    </Card>
  );
}
