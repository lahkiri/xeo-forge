  ...
  return (
    <Card className="mb-6">
      <div className="mb-5 flex flex-wrap items-start justify-between gap-3">
        <div>
          <p className="text-meta font-semibold uppercase tracking-[0.18em] text-signal-plan">PROFILE STUDIO</p>
          <h2 className="mt-1 font-semibold">Reusable agent roles</h2>
          <p className="mt-1 max-w-2xl text-meta leading-5 text-content-muted">Create a focused operating mode for building, research, analysis, or operations. Profiles guide behavior; they never grant permissions or bypass approval.</p>
        </div>
        <span className="rounded-full bg-signal-plan/10 px-2.5 py-1 text-micro text-signal-plan">{profiles.filter((p) => p.enabled).length} enabled</span>
      </div>
      ...
      <form onSubmit={create} className="grid gap-3 md:grid-cols-[1fr_150px]">
        <input value={name} onChange={(e) => setName(e.target.value)} placeholder="Profile name, e.g. Senior Builder" className="rounded-md border border-line bg-ink-700/60 px-3 py-2 text-ui outline-none placeholder:text-content-muted focus:border-signal-plan/45" />
        <select value={kind} onChange={(e) => setKind(e.target.value as AgentProfileKind)} className="rounded-md border border-line bg-[#111419] px-3 py-2 text-ui text-content-secondary outline-none focus:border-signal-plan/45">{kinds.map((item) => <option key={item} value={item}>{item}</option>)}</select>
        <input value={description} onChange={(e) => setDescription(e.target.value)} placeholder="Short description" className="rounded-md border border-line bg-ink-700/60 px-3 py-2 text-ui outline-none placeholder:text-content-muted focus:border-signal-plan/45 md:col-span-2" />
        <textarea value={instructions} onChange={(e) => setInstructions(e.target.value)} rows={3} placeholder="You are a senior engineer. Prefer small reversible changes, run tests, and explain trade-offs…" className="w-full resize-y rounded-md border border-line bg-ink-700/60 px-3 py-2 text-ui leading-6 outline-none placeholder:text-content-muted focus:border-signal-plan/45 md:col-span-2" />
        <div className="md:col-span-2"><Button type="submit" disabled={busy || !name.trim() || !instructions.trim()}>Create profile</Button></div>
      </form>
      {profiles.length > 0 && <div className="mt-5 grid gap-3 md:grid-cols-2">{profiles.map((profile) => <div key={profile.id} className="rounded-control border border-line-subtle bg-ink-700/60 p-4"><div className="flex items-start justify-between gap-3"><div><p className="text-ui font-medium text-content-primary">{profile.name}</p><p className="mt-1 text-micro uppercase tracking-wider text-content-muted">{profile.kind} · v{profile.version}</p></div><span className={`rounded-full px-2 py-1 text-micro ${profile.enabled ? 'bg-green-500/15 text-green-300' : 'bg-white/10 text-content-muted'}`}>{profile.enabled ? 'enabled' : 'disabled'}</span></div><p className="mt-3 line-clamp-3 text-meta leading-5 text-content-muted">{profile.description || profile.instructions}</p><div className="mt-3 flex gap-2"><Button variant="ghost" disabled={busy} onClick={() => void toggle(profile)}>{profile.enabled ? 'Disable' : 'Enable'}</Button><Button variant="ghost" disabled={busy} onClick={() => void remove(profile)}>Delete</Button></div></div>)}</div>}
    </Card>
  );
}
