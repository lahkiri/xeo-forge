  ...
  return (
    <Card className="mb-6">
      <div className="mb-5 flex flex-wrap items-start justify-between gap-3">
        <div>
          <p className="text-meta font-semibold uppercase tracking-[0.18em] text-cyan-300">SKILL STUDIO</p>
          <h2 className="mt-1 font-semibold">Reusable workflows</h2>
          <p className="mt-1 max-w-2xl text-meta leading-5 text-content-muted">Turn a repeatable process into a selectable skill. A skill provides workflow guidance and an optional profile; approval gates, mode policy, and workspace-confined file operations remain enforced by Xeo.</p>
        </div>
      ...
      <form onSubmit={create} className="grid gap-3 md:grid-cols-[1fr_150px]">
        <input value={name} onChange={(e) => setName(e.target.value)} placeholder="Skill name, e.g. Ship a production feature" className="rounded-md border border-line bg-ink-700/60 px-3 py-2 text-ui outline-none placeholder:text-content-muted focus:border-cyan-400/50" />
        <select value={kind} onChange={(e) => setKind(e.target.value as AgentSkillKind)} className="rounded-md border border-line bg-[#111419] px-3 py-2 text-ui text-content-secondary outline-none focus:border-cyan-400/50">{kinds.map((item) => <option key={item} value={item}>{item}</option>)}</select>
        <input value={description} onChange={(e) => setDescription(e.target.value)} placeholder="Short description" className="rounded-md border border-line bg-ink-700/60 px-3 py-2 text-ui outline-none placeholder:text-content-muted focus:border-cyan-400/50 md:col-span-2" />
        <textarea value={instructions} onChange={(e) => setInstructions(e.target.value)} rows={4} placeholder="1. Inspect the repository. 2. Propose a plan. 3. Implement in small slices. 4. Run tests and report evidence…" className="w-full resize-y rounded-md border border-line bg-ink-700/60 px-3 py-2 text-ui leading-6 outline-none placeholder:text-content-muted focus:border-cyan-400/50 md:col-span-2" />
        <div className="flex flex-wrap items-center gap-3 md:col-span-2"><select value={profileId} onChange={(e) => setProfileId(e.target.value)} className="rounded-md border border-line bg-[#111419] px-3 py-2 text-meta text-content-secondary outline-none focus:border-cyan-400/50"><option value="">No profile override</option>{profiles.filter((p) => p.enabled).map((p) => <option key={p.id} value={p.id}>{p.name}</option>)}</select><Button type="submit" disabled={busy || !name.trim() || !instructions.trim()}>Create skill</Button></div>
      </form>
