  ...
  return (
    <Card className="mb-6">
      <div className="mb-5 flex flex-wrap items-start justify-between gap-3">
        ...
        <span className="rounded-full bg-signal-run/10 px-2.5 py-1 text-micro text-signal-run">{skills.filter((s) => s.enabled).length} enabled</span>
      </div>
      ...
      <form onSubmit={create} className="grid gap-3 md:grid-cols-[1fr_150px]">
        <input value={name} onChange={(e) => setName(e.target.value)} placeholder="Skill name, e.g. Ship a production feature" className="rounded-md border border-line bg-ink-700/60 px-3 py-2 text-ui outline-none placeholder:text-content-muted focus:border-signal-run/45" />
        <select value={kind} onChange={(e) => setKind(e.target.value as AgentSkillKind)} className="rounded-md border border-line bg-[#111419] px-3 py-2 text-ui text-content-secondary outline-none focus:border-signal-run/45">{kinds.map((item) => <option key={item} value={item}>{item}</option>)}</select>
        <input value={description} onChange={(e) => setDescription(e.target.value)} placeholder="Short description" className="rounded-md border border-line bg-ink-700/60 px-3 py-2 text-ui outline-none placeholder:text-content-muted focus:border-signal-run/45 md:col-span-2" />
