      ...
      <div className="relative grid w-full max-w-5xl gap-8 lg:grid-cols-[1.1fr_420px] lg:items-center">
        <section className="hidden px-4 lg:block">
          <div className="flex items-center gap-3">
            <div className="flex h-11 w-11 items-center justify-center rounded-modal border border-signal-run/25 bg-signal-run/08 text-xl text-signal-run shadow-[0_0_32px_rgba(103,232,249,0.12)]">◇</div>
            <div>
        ...
        <Card className="border-signal-run/10 bg-white/[0.045] p-6 shadow-2xl shadow-black/30 sm:p-8">
          <div className="mb-7 lg:hidden">
            <div className="flex items-center gap-3">
              <div className="flex h-10 w-10 items-center justify-center rounded-modal border border-signal-run/25 bg-signal-run/08 text-lg text-signal-run">◇</div>
              <div>
          ...
          <p className="mt-2 text-sm leading-6 text-content-muted">Your profiles, skills, memory, and run ledger are waiting.</p>
          <form onSubmit={onSubmit} className="mt-7 space-y-4">
            <label className="block space-y-2">
              <span className="block text-xs font-medium text-content-secondary">Email</span>
              <input type="email" autoComplete="email" required value={email} onChange={(e) => setEmail(e.target.value)} className="w-full rounded-panel border border-line bg-[#070b12]/80 px-3.5 py-3 text-sm text-content-primary outline-none transition placeholder:text-content-muted focus:border-signal-run/50 focus:ring-4 focus:ring-cyan-300/[0.08]" />
            </label>
            <label className="block space-y-2">
              <span className="block text-xs font-medium text-content-secondary">Password</span>
              <input type="password" autoComplete="current-password" required value={password} onChange={(e) => setPassword(e.target.value)} className="w-full rounded-panel border border-line bg-[#070b12]/80 px-3.5 py-3 text-sm text-content-primary outline-none transition placeholder:text-content-muted focus:border-signal-run/50 focus:ring-4 focus:ring-cyan-300/[0.08]" />
            </label>
            {error && <p role="alert" className="rounded-panel border border-red-300/15 bg-signal-fail/08 px-3 py-2 text-sm text-signal-fail">{error}</p>}
          ...
          <p className="mt-6 text-center text-sm text-content-muted">Need an account? <Link href="/register" className="font-medium text-cyan-300 hover:text-signal-run">Create one</Link></p>
        </Card>
      </div>
    </main>
  );
